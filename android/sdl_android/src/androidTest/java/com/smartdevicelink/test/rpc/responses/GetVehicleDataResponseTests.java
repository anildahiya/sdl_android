...
public class GetVehicleDataResponseTests extends BaseRpcTests{
    ...
    public void testJson() {
		...
		try {
			...
			//SEAT_OCCUPANCY
			seatOccupancyObj.put(SeatOccupancy.KEY_SEATS_OCCUPIED, VehicleDataHelper.SEAT_OCCUPANCY.getSeatsOccupied());
			seatOccupancyObj.put(SeatOccupancy.KEY_SEATS_BELTED, VehicleDataHelper.SEAT_OCCUPANCY.getSeatsBelted());
			...
			while (iterator.hasNext()) {
				...
				if (key.equals(GetVehicleDataResponse.KEY_TIRE_PRESSURE)) {
					...
					JSONObject seatOccupancyObjReference = JsonUtils.readJsonObjectFromJsonObject(reference, key);
					JSONObject seatOccupancyObjTest = JsonUtils.readJsonObjectFromJsonObject(underTest, key);

					assertTrue("JSON value didn't match expected value for key \"" + key + "\".",
							Validator.validateSeatOccupancy(
									new SeatOccupancy(JsonRPCMarshaller.deserializeJSONObject(seatOccupancyObjReference)),
									new SeatOccupancy(JsonRPCMarshaller.deserializeJSONObject(seatOccupancyObjTest))));
				}
	...
	public void testRpcValues(){		
		...
		assertEquals(Test.MATCH, VehicleDataHelper.HANDS_OFF_STEERING, ( (GetVehicleDataResponse) msg ).getHandsOffSteering());
    ...
    public void testJsonConstructor () {
		...
		try {
			...
			Boolean handsOffSteering = JsonUtils.readBooleanFromJsonObject(parameters, GetVehicleDataResponse.KEY_HANDS_OFF_STEERING);
			assertEquals(Test.MATCH, handsOffSteering, cmd.getHandsOffSteering());
			...
			assertTrue(Test.TRUE, Validator.validateSeatOccupancy(seatOccupancy, cmd.getSeatOccupancy()) );

			JSONArray windowStatusArray = JsonUtils.readJsonArrayFromJsonObject(parameters, GetVehicleDataResponse.KEY_WINDOW_STATUS);
			List<WindowStatus> windowStatusList = new ArrayList<>();
			for (int index = 0; index < windowStatusArray.length(); index++) {
				WindowStatus windowStatus = new WindowStatus(JsonRPCMarshaller.deserializeJSONObject((JSONObject) windowStatusArray.get(index)));
