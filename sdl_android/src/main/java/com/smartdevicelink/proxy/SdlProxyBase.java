	...
	//Initialize _systemCapabilityManager here.
		...
		synchronized(CONNECTION_REFERENCE_LOCK) {
			if(_transportConfig.getTransportType().equals(TransportType.MULTIPLEX)){
				this.sdlSession = new SdlSession2(_interfaceBroker,(MultiplexTransportConfig)_transportConfig, _applicationName, _appID);
			}else{
				this.sdlSession = SdlSession.createSession((byte)getProtocolVersion().getMajor(),_interfaceBroker, _transportConfig, _applicationName, _appID);
			}
		}
