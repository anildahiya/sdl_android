...
import com.smartdevicelink.service.proxy.RPCRequest;
import com.smartdevicelink.service.streaming.AbstractPacketizer;
import com.smartdevicelink.service.streaming.IStreamListener;
import com.smartdevicelink.service.streaming.StreamPacketizer;
import com.smartdevicelink.service.streaming.StreamRPCPacketizer;
...
public class SdlConnection implements IProtocolListener, ITransportListener, IStreamListener  {
	...
	public CopyOnWriteArrayList<SdlSession> getListenerList(){
		return listenerList;
	}
	...
	public void onTransportDisconnected(String info) {
		...
		_connectionListener.onTransportDisconnected(info, this);
	}

	@Override
	public void onTransportError(String info, Exception e) {
		// Pass directly to connection listener
		_connectionListener.onTransportError(info, e, this);
	}
	...
	public void onProtocolMessageReceived(ProtocolMessage msg) {
		_connectionListener.onProtocolMessageReceived(msg, this);
	}

	@Override
	public void onProtocolSessionStarted(SessionType sessionType,
			byte sessionID, byte version, String correlationID) {
		_connectionListener.onProtocolSessionStarted(sessionType, sessionID, version, correlationID, this);
	}

	@Override
	public void onProtocolSessionNACKed(SessionType sessionType,
			byte sessionID, byte version, String correlationID) {
		_connectionListener.onProtocolSessionNACKed(sessionType, sessionID, version, correlationID, this);
	}

	@Override
	public void onProtocolSessionEnded(SessionType sessionType, byte sessionID,
			String correlationID) {
		_connectionListener.onProtocolSessionEnded(sessionType, sessionID, correlationID, this);
	}

	@Override
	public void onProtocolError(String info, Exception e) {
		_connectionListener.onProtocolError(info, e, this);
	}
	...
	private class InternalMsgDispatcher implements ISdlConnectionListener {
		...
		public void onTransportDisconnected(String info, SdlConnection sdlConnection) {
			for (SdlSession session : listenerList) {
				session.onTransportDisconnected(info, session.getSessionId(), sdlConnection);
			}
		}
		...
		public void onTransportError(String info, Exception e, SdlConnection sdlConnection) {
				for (SdlSession session : listenerList) {
						session.onTransportError(info, e, session.getSessionId(), sdlConnection);
				}
		}
		...
		public void onProtocolMessageReceived(ProtocolMessage msg, SdlConnection sdlConnection) {
			...
			if (session != null) {
				session.onProtocolMessageReceived(msg, sdlConnection);
			}
		}
		...
		public void onProtocolSessionStarted(SessionType sessionType,
			...
			for (SdlSession session : listenerList) {
				if (session.getSessionId() == 0 || sessionType == SessionType.NAV) {
					session.onProtocolSessionStarted(sessionType, sessionID, version, correlationID, sdlConnection);
		...
		public void onProtocolSessionEnded(SessionType sessionType,
				byte sessionID, String correlationID, SdlConnection sdlConnection) {
			SdlSession session = findSessionById(sessionID);
			if (session != null) {
				session.onProtocolSessionEnded(sessionType, sessionID, correlationID, sdlConnection);
			}
		}
		...
		public void onProtocolError(String info, Exception e, SdlConnection sdlConnection) {
			for (SdlSession session : listenerList) {
				session.onProtocolError(info, e, sdlConnection);
			}
		}
		...
		public void onProtocolSessionNACKed(SessionType sessionType,
				byte sessionID, byte version, String correlationID, SdlConnection sdlConnection) {
			for (SdlSession session : listenerList) {
				session.onProtocolSessionNACKed(sessionType, sessionID, version, correlationID, sdlConnection);
			}			
		}
